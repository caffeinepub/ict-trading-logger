...
} from "../utils/analytics/timeBuckets";
import { computeToolImpact } from "../utils/analytics/toolImpact";
import {
  type Session,
  filterTrades,
  getCompletedTrades,
} from "../utils/analytics/tradeScope";
...
export default function Analytics() {
  ...
  const filteredTrades = useMemo(() => {
    return filterTrades(trades, {
      ...
      adherenceThreshold: adherenceFilterEnabled
        ? adherenceThreshold / 100
        : undefined,
    });
  }, [
    trades,
  ...
  const equityCurve = useMemo(
    () => computeEquityCurve(filteredTrades),
    [filteredTrades],
  );
  ...
  const sessionMetrics = useMemo(
    () => computeSessionMetrics(filteredTrades),
    [filteredTrades],
  );
  ...
  const bracketMetrics = useMemo(
    () => computeBracketMetrics(filteredTrades),
    [filteredTrades],
  );
  const biasMetrics = useMemo(
    () => computeBiasMetrics(filteredTrades, models),
    [filteredTrades, models],
  );
  const hourBuckets = useMemo(
    () => computeHourBuckets(filteredTrades),
    [filteredTrades],
  );
  const weekdayBuckets = useMemo(
    () => computeWeekdayBuckets(filteredTrades),
    [filteredTrades],
  );
  const toolImpact = useMemo(
    () => computeToolImpact(filteredTrades, models),
    [filteredTrades, models],
  );
  const volatilityMetrics = useMemo(
    () => computeVolatilityMetrics(filteredTrades),
    [filteredTrades],
  );
  const monteCarloResult = useMemo(
    () => runMonteCarloSimulation(filteredTrades),
    [filteredTrades],
  );
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-end">
          ...
          <div className="flex-1 space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) => setSelectedSession(v as Session)}
            >
              <SelectTrigger className="w-full md:w-[200px]">
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Card>
                ...
                <CardContent>
                  <div
                    className={`text-2xl font-bold ${metrics.totalPL >= 0 ? "text-green-500" : "text-red-500"}`}
                  >
          ...
          <TabsContent value="calendar" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Daily aggregated performance metrics. Click a day to view
          ...
          <TabsContent value="sessions" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Win rate, P/L, and average R by trading session
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                ...
                <Card key={session.session}>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${session.totalPL >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
          ...
          <TabsContent value="adherence" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Comparing trades with adherence ≥ {adherenceThreshold}% vs all
                  trades
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">
                        High Adherence (≥{adherenceThreshold}%)
                      </p>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-sm">Trades</span>
                          <Badge variant="outline">
                            {adherenceComparison.filteredTrades}
                          </Badge>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm">Win Rate</span>
                          <span className="font-bold">
                            {adherenceComparison.filteredWinRate.toFixed(1)}%
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm">Total P/L</span>
                          <span
                            className={`font-bold ${adherenceComparison.filteredPL >= 0 ? "text-green-500" : "text-red-500"}`}
                          >
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-sm">Trades</span>
                          <Badge variant="outline">
                            {adherenceComparison.allTrades}
                          </Badge>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm">Win Rate</span>
                          <span className="font-bold">
                            {adherenceComparison.allWinRate.toFixed(1)}%
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm">Total P/L</span>
                          <span
                            className={`font-bold ${adherenceComparison.allPL >= 0 ? "text-green-500" : "text-red-500"}`}
                          >
                ...
                <div className="mt-6 pt-6 border-t border-border">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="flex justify-between items-center p-3 rounded-lg bg-muted/50">
                      ...
                      <span
                        className={`font-bold ${adherenceComparison.winRateDelta >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {adherenceComparison.winRateDelta >= 0 ? "+" : ""}
                        {adherenceComparison.winRateDelta.toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-3 rounded-lg bg-muted/50">
                      <span className="text-sm">P/L Difference</span>
                      <span
                        className={`font-bold ${adherenceComparison.plDelta >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {adherenceComparison.plDelta >= 0 ? "+" : ""}$
                        {adherenceComparison.plDelta.toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <TabsContent value="brackets" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Hit rates and realized R by bracket level
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                ...
                <Card key={bracket.bracketIndex}>
                  <CardHeader>
                    <CardTitle className="text-lg">
                      Bracket {bracket.bracketIndex + 1}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {bracket.avgRealizedR.toFixed(2)}
                      </span>
                    </div>
                  </CardContent>
                </Card>
          ...
          <TabsContent value="bias" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Win rate and P/L by higher timeframe bias
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                ...
                <Card key={bias.bias}>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-medium ${bias.totalPL >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
          ...
          <TabsContent value="tools" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Performance impact by tool type and zone
                </CardDescription>
              </CardHeader>
              <CardContent>
                {toolImpact.length === 0 ? (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className="text-lg font-bold">
                            {tool.winRate.toFixed(1)}%
                          </p>
                          <p
                            className={`text-sm ${tool.avgPL >= 0 ? "text-green-500" : "text-red-500"}`}
                          >
          ...
          <TabsContent value="advanced" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Win rate and P/L by market volatility
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
