...
import type { ModelCondition, Trade } from "../types";
import {
...
export default function Trades() {
  ...
  const [preloadedObservations, setPreloadedObservations] = useState<
    ModelCondition[]
  >([]);
  ...
  const handleSelectModelFromIdentifier = (
    modelId: string,
    observations: ModelCondition[],
  ) => {
    setEditingTrade(null);
  ...
  const getTradeStatus = (
    ...
    const hasOutcome =
      ...
      trade.bracket_order_outcomes.length > 0;
  ...
  const uniqueAssets = Array.from(
    new Set(trades.map((t) => t.asset).filter((a): a is string => !!a)),
  );
          ...
          <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm sm:text-base text-muted-foreground break-words">
                Record the outcome for {outcomeTrade.asset}{" "}
                {outcomeTrade.direction.toUpperCase()}
              </p>
            </div>
            <Button
              variant="outline"
    ...
    <ErrorBoundary>
      <div className="container py-8 px-4 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button
            onClick={handleOpenNewTradeForm}
        ...
        <div className="flex flex-col sm:flex-row gap-4 flex-wrap">
          ...
          <Select
            value={sortBy}
            onValueChange={(v) => setSortBy(v as "date" | "rr")}
          >
            <SelectTrigger className="w-full sm:w-[200px]">
        ...
        {filteredTrades.length === 0 ? (
          ...
          <div className="grid gap-4">
            {filteredTrades.map((trade) => {
              ...
              const tradePL = status.isOpen
                ? 0
                : computeTradePLFromOutcomes(trade);
              const tradeRR = status.isOpen
                ? 0
                : computeTradeRRFromOutcomes(trade);
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      <div className="space-y-1 min-w-0 flex-1">
                        ...
                        <CardDescription className="break-words">
                          ...
                          {format(
                            new Date(Number(trade.created_at) / 1000000),
                            "PPP",
                          )}
                        </CardDescription>
                      </div>
