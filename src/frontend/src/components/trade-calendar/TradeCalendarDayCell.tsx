    ...
    >
      ...
      {hasTrades && (
        <div className="space-y-1 text-xs">
          <div
            className={`font-semibold ${aggregates.totalPLDollar >= 0 ? "text-green-500" : "text-red-500"}`}
          >
            ${aggregates.totalPLDollar.toFixed(2)}
          </div>
          <div className="text-muted-foreground">
            {aggregates.totalPLPercent >= 0 ? "+" : ""}
            {aggregates.totalPLPercent.toFixed(1)}%
          </div>
          <div className="text-muted-foreground">
            {aggregates.netR >= 0 ? "+" : ""}
            {aggregates.netR.toFixed(2)}R
          </div>
          <div className="text-muted-foreground">
            {aggregates.tradeCount} trade
            {aggregates.tradeCount !== 1 ? "s" : ""}
          </div>
        </div>
      )}
    </div>
