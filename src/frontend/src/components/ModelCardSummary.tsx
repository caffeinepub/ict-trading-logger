...
export default function ModelCardSummary({
  ...
  const renderConciseToolList = (tools: ToolConfig[]) => {
    if (tools.length === 0) {
      return (
        <p className="text-sm text-muted-foreground italic">No tools defined</p>
      );
    }
    ...
    return (
      <div className="space-y-2">
        {tools.map((tool) => {
          ...
          const typeKeys = Object.keys(properties).filter(
            (k) => k.toLowerCase().includes("type") && k !== "type",
          );
          ...
          const directionKeys = Object.keys(properties).filter(
            (k) =>
          ...
          const timeframeUnitKeys = Object.keys(properties).filter((k) =>
            k.toLowerCase().includes("timeframeunit"),
          );
          const timeframeValueKeys = Object.keys(properties).filter((k) =>
            k.toLowerCase().includes("timeframevalue"),
          );
          ...
          if (
            properties.structuralState &&
            properties.structuralState !== "Regular"
          ) {
                ...
                {narrativeTools.length > 0 && (
                  <div>
                    ...
                    <p className="text-muted-foreground leading-relaxed">
                      {narrativeTools
                        .map((tool) => getToolName(tool.type))
                        .join(", ")}
                    </p>
                  </div>
                )}
