...
import type { Model, ModelCondition } from "../types";
import {
  calculateModelAdherence,
  mapObservationsToModelConditions,
} from "../utils/setupIdentifier/adherence";

interface SetupIdentifierProps {
  onSelectModel: (
    modelId: string,
    qualifyingConditions: ModelCondition[],
  ) => void;
}
...
export default function SetupIdentifier({
  onSelectModel,
}: SetupIdentifierProps) {
  ...
  useMemo(() => {
    if (
      ...
      // Add narrative tools
      for (const toolType of toolsByZone.narrative) {
      ...
      // Add framework tools
      for (const toolType of toolsByZone.framework) {
        initialObservations.push({
          toolType,
          zone: "framework",
          isSelected: false,
      ...
      // Add execution tools
      for (const toolType of toolsByZone.execution) {
        initialObservations.push({
          toolType,
          zone: "execution",
          isSelected: false,
  ...
  const clearAllObservations = () => {
    setObservations((prev) =>
      prev.map((obs) => ({ ...obs, isSelected: false })),
    );
  };
  ...
  const modelMatches = useMemo((): ModelMatch[] => {
    ...
    const matches: ModelMatch[] = models.map((model) => {
      ...
      const modelConditions = mapObservationsToModelConditions(
        model,
        selectedObs,
      );
      ...
      const narrativeConditions = modelConditions.filter(
        (c) => c.zone === "narrative",
      );
      const frameworkConditions = modelConditions.filter(
        (c) => c.zone === "framework",
      );
      const executionConditions = modelConditions.filter(
        (c) => c.zone === "execution",
      );

      const narrativeMatch =
        narrativeConditions.length > 0
          ? calculateModelAdherence(narrativeConditions)
          : 0;

      const frameworkMatch =
        frameworkConditions.length > 0
          ? calculateModelAdherence(frameworkConditions)
          : 0;

      const executionMatch =
        executionConditions.length > 0
      ...
      for (const condition of modelConditions) {
        if (!condition.isChecked) {
          missingTools.push(
            `${condition.description.split(" - ")[0]} (${condition.zone})`,
          );
        }
      }
    ...
    const threshold = Number.parseFloat(adherenceThreshold) || 0;
    return matches
  ...
  const selectedObservationsCount = observations.filter(
    (o) => o.isSelected,
  ).length;
            ...
            <div className="space-y-4">
              ...
              <ScrollArea className="h-[400px] rounded-md border p-4">
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {
                          observationsByZone.narrative.filter(
                            (o) => o.isSelected,
                          ).length
                        }{" "}
                        selected
                    ...
                    {toolsByZone.narrative.length === 0 ? (
                      <p className="text-xs text-muted-foreground pl-2">
                        No narrative tools in your models
                      </p>
                    ) : (
                      <div className="grid grid-cols-2 gap-2 pl-2">
                        {toolsByZone.narrative.map((toolType) => {
                          const obs = observationsByZone.narrative.find(
                            (o) => o.toolType === toolType,
                          );
                          if (!obs) return null;

                          return (
                            <div
                              key={`narrative-${toolType}`}
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={() =>
                                  toggleObservation(toolType, "narrative")
                                }
                              />
                              <Label
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <Badge className="bg-purple-500">Setup</Badge>
                      <span className="text-xs text-muted-foreground">
                        {
                          observationsByZone.framework.filter(
                            (o) => o.isSelected,
                          ).length
                        }{" "}
                        selected
                    ...
                    {toolsByZone.framework.length === 0 ? (
                      <p className="text-xs text-muted-foreground pl-2">
                        No framework tools in your models
                      </p>
                    ) : (
                      <div className="grid grid-cols-2 gap-2 pl-2">
                        {toolsByZone.framework.map((toolType) => {
                          const obs = observationsByZone.framework.find(
                            (o) => o.toolType === toolType,
                          );
                          if (!obs) return null;
                          ...
                          return (
                            ...
                            >
                              <Checkbox
                                ...
                                checked={obs.isSelected}
                                onCheckedChange={() =>
                                  toggleObservation(toolType, "framework")
                                }
                              />
                              <Label
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {
                          observationsByZone.execution.filter(
                            (o) => o.isSelected,
                          ).length
                        }{" "}
                        selected
                    ...
                    {toolsByZone.execution.length === 0 ? (
                      <p className="text-xs text-muted-foreground pl-2">
                        No execution tools in your models
                      </p>
                    ) : (
                      <div className="grid grid-cols-2 gap-2 pl-2">
                        {toolsByZone.execution.map((toolType) => {
                          const obs = observationsByZone.execution.find(
                            (o) => o.toolType === toolType,
                          );
                          if (!obs) return null;
                          ...
                          return (
                            ...
                            >
                              <Checkbox
                                ...
                                checked={obs.isSelected}
                                onCheckedChange={() =>
                                  toggleObservation(toolType, "execution")
                                }
                              />
                              <Label
              ...
              <div className="space-y-4">
                ...
                {modelMatches.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p>
                      No models match your observations above{" "}
                      {adherenceThreshold}% threshold
                    </p>
                    <p className="text-xs mt-2">
                      Try lowering the threshold or selecting different tools
                    </p>
                  </div>
                ) : (
                                ...
                                <div className="space-y-1">
                                  <div className="flex items-center justify-between text-xs">
                                    ...
                                    <span className="font-medium">
                                      {match.narrativeMatch.toFixed(0)}%
                                    </span>
                                  </div>
                                  <Progress
                                    value={match.narrativeMatch}
                                    className="h-1.5"
                                  />
                                </div>
                              )}
                              ...
                              {match.frameworkTotal > 0 && (
                                <div className="space-y-1">
                                  <div className="flex items-center justify-between text-xs">
                                    ...
                                    <span className="font-medium">
                                      {match.frameworkMatch.toFixed(0)}%
                                    </span>
                                  </div>
                                  <Progress
                                    value={match.frameworkMatch}
                                    className="h-1.5"
                                  />
                                </div>
                              )}
                              ...
                              {match.executionTotal > 0 && (
                                <div className="space-y-1">
                                  <div className="flex items-center justify-between text-xs">
                                    ...
                                    <span className="font-medium">
                                      {match.executionMatch.toFixed(0)}%
                                    </span>
                                  </div>
                                  <Progress
                                    value={match.executionMatch}
                                    className="h-1.5"
                                  />
                                </div>
                              )}
