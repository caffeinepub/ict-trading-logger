...
export function getCompletedTradesWithCloseTime(trades: Trade[]): Trade[] {
  return trades.filter(
    (t) =>
      t.is_completed && t.close_time !== undefined && t.close_time !== null,
  );
}
...
export function aggregateTradesByDay(
  ...
  for (const trade of completedTrades) {
    ...
    const riskAmount = Math.abs(
      (trade.bracket_order.entry_price -
        trade.bracket_order.primary_stop_loss) *
...
export function getMonthCalendarGrid(
  year: number,
  month: number,
  dayAggregates: Map<string, DayAggregates>,
): CalendarDay[] {
