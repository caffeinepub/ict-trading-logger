...
export function computeBiasMetrics(
  ...
  for (const [bias, biasTrades] of biasGroups) {
    if (biasTrades.length === 0) continue;

    const wins = biasTrades.filter((t) => isTradeWinner(t)).length;
    const winRate = (wins / biasTrades.length) * 100;
    const totalPL = biasTrades.reduce(
      (sum, t) => sum + computeTradePLFromOutcomes(t),
      0,
    );
    const avgR =
      biasTrades.reduce((sum, t) => sum + computeTradeRRFromOutcomes(t), 0) /
      biasTrades.length;
